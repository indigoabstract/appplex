...
vec3 star2(vec2 position, float BLADES) {
	float blade = clamp(pow(sin(atan(position.y,position.x )*BLADES + u_v1_time*.5)+BIAS, 8.0), 0.0, 1.0);
...
void main( void ) {
	...
	vec2   pos = v_v2_tex_coord * u_v2_dim - ORIGIN ;
	float dist = length(pos) / u_v2_dim.y;
	vec2 coord = vec2(pow(dist, 0.1), atan(pos.x, pos.y) / (3.1415926*2.0));
	
	// Nebulous cloud
	vec3 color = cloud(pos/u_v2_dim);
	
	// Background stars
	float a = pow((1.0-dist),20.0);
	float t = u_v1_time*-.05;
	...
	color += asteroids(pos/u_v2_dim.x);
