// http://glsl.heroku.com/e#14718.0
#ifdef GL_ES
...
void main( void ) {
	...
	for(int i=1;i<50;i++){
		vec2 newp=p;
		newp.x+=(0.11/float(i))*sin(float(i)*p.y*4.0+u_v1_time*0.3)*2.2+1.2;
		newp.y+=(0.1/float(i))*sin(float(i)*p.x*3.0+u_v1_time*0.3)*1.2-1.2;
