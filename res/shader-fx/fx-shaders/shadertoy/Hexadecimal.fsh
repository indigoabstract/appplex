// https://www.shadertoy.com/view/MdfSzr

// iChannel0: m2

// 42 45 53 55 52 45 54 4F 44 52 49 4E 4B 59 4F 55 52 4F 56 41 4C 54 49 4E 45
// xTibor / 2014
...
void main(void) {
	...
	uv *= 12.0 + beat() * 3.0;	
	uv /= dot(uv, vec2(sin(iGlobalTime * 0.34), sin(iGlobalTime * 0.53))) * 0.1 + 2.0; // Tilting
	uv -= iGlobalTime / 1.0;	
	
	bool b = digit(fract(uv), int(mod(floor(uv.x) * floor(uv.y), 16.0)), 0.1);		
