...
// This is the sampling approach we used to generate ocean geometry (see code comments). The red line gives a density curve. When intersected with the binary tree it generates sample locations at the desired density. Mouse to move.
