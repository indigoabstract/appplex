// https://www.shadertoy.com/view/llBGR1
...
{
    float m = 9999.0;
    vec4 audio = texture2D(iChannel2, vec2(0.0, 0.0));
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec4 audio = texture2D(iChannel2, vec2(1.0, 0.0));
	...
	if(h)
    {
       ...
       color += textureCube(iChannel1, -grad);
    ...
    color += aColor * pow(clamp(d, 0.0, 1.0), 1.0);
