// https://www.shadertoy.com/view/XdXXzB
...
float noise (vec2 co) {
  return length (texture2D (iChannel2, co));
}
...
vec4 fbm3d4v (vec3 p, float s) {	
	return 
		compute (p.xy, iGlobalTime/s) * abs (p.z) +
		compute (p.xz, iGlobalTime/s) * abs (p.y) +
		compute (p.yz, iGlobalTime/s) * abs (p.x);
}
...
void main (void) {
	...
	vec3 rn = rd;
	vec3 dr = fbm4v (uv/64.0 + sin (iGlobalTime/128.0)).xyz - 0.5;
	//rd = normalize (rd + dr/32.0);
	
	vec4 sp = SPHERE + 					
		vec4 (0.0, 1.0, 0.0, 0.0)*sin (iGlobalTime); 							
	...
	vec4 color = textureCube (iChannel0, rn);
	...
	if (d > 0.0) {
		...
		vec4 s0 = compute (spherical (pn0.zxy).yz/2.0, iGlobalTime/8.0);
		vec4 s1 = compute (spherical (pn1.zxy).yz/2.0, iGlobalTime/8.0);
		vec4 c0 = textureCube (iChannel1, r0);
		vec4 c1 = textureCube (iChannel1, r1);		
