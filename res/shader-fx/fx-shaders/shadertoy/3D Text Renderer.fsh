// https://www.shadertoy.com/view/XdsSzl
#define STEPS 16
...
{	
	...
	letters = min(letters,S(p.xy-vec2(-2.0,0.5)));
	letters = min(letters,H(p.xy-vec2(-1.5,0.5)));
	letters = min(letters,A(p.xy-vec2(-1.0,0.5)));
	letters = min(letters,D(p.xy-vec2(-0.5,0.5)));
	letters = min(letters,E(p.xy-vec2( 0.0,0.5)));
	letters = min(letters,R(p.xy-vec2( 0.5,0.5)));
	letters = min(letters,T(p.xy-vec2( 1.0,0.5)));
	letters = min(letters,O(p.xy-vec2( 1.5,0.5)));
	letters = min(letters,Y(p.xy-vec2( 2.0,0.5)));
