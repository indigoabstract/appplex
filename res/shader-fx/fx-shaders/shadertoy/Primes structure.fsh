...
// SPACE= no zoom 3 way to display the distribution of primes. after "discovering numbers structures through images" by Jean-Paul Delahaye in "Pour la Science", French edition of Scientific American, Feb 2014 http://www.lifl.fr/~delahaye/pls/2014/243.pdf
...
void main(void) {
	...
	vec2 uv;
	uv.x = float(int(gl_FragCoord.x)-int(iResolution.x/2.))*zoom;
	uv.y = float(int(gl_FragCoord.y)-int(iResolution.y/2.))*zoom;
	int i;	
	...
	{ 						 // i = length along a squared spiral 
		...
		//gl_FragColor = vec4(mod(float(i),2.)); return;	
		//gl_FragColor = vec4(float(i)/40000.); return;	
	}	
	else 
...
#       define RAD 1. // 4. // zoom
    	float r = length(uv)/RAD, a = atan(uv.y, uv.x),
			  ...
			  l = t*t;// int (a/2PI da)
		//gl_FragColor = vec4(sin(2.*PI*s)); return;
		//gl_FragColor = vec4(sin(l/4.))+vec4(max(0.,sin(s)),0.,0.,0.);return;
 		i = int(l);
	}

		gl_FragColor = vec4(prime(i));
}
