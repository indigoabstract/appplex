...
// Trying to simulate temporally coherent oil paint flow
#ifdef GL_ES
...
{
	q.x = fbm( p  +0.00*iGlobalTime * 2.); // @SLIDER: 5. could represent velocity of water
	q.y = fbm( p + vec2(1.0));
	
	r.x = fbm( p +1.0*q + vec2(1.7,9.2)+0.15*iGlobalTime * 2. );
	r.y = fbm( p+ 1.0*q + vec2(8.3,2.8)+0.126*iGlobalTime * 2.);
	//r = cMul(q,q+0.1*time);
