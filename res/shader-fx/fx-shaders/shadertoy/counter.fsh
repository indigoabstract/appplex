// https://www.shadertoy.com/view/XslGD7
...
void main(void)
{
	...
	if (c>0) 
		gl_FragColor = vec4(0.5+0.5*sin(iGlobalTime),uv,1.0);     // draw set pixels
	//else if (c==0)
	//	gl_FragColor = vec4(0.*uv,0.5-0.5*sin(iGlobalTime),1.0);  // draw digit background
	else
		gl_FragColor = vec4(uv,0.5-0.5*sin(iGlobalTime),1.0);     // draw background
}
