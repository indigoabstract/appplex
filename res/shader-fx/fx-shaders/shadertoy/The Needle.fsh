// https://www.shadertoy.com/view/XsXGR4
...
void main(void)
{
	...
	vec2 c=(.5-gl_FragCoord.xy*d)*scale;
	// d is used later for anti-aliasing
	d*=scale;
	vec4 f;
	
	// Angle of the needle from top in radians
	float o=1.5*sin(3.*sin(iGlobalTime*.271)+.2*sin(iGlobalTime*7.1)+iGlobalTime*.1); 
	...
	gl_FragColor=mix(vec4(0.2),f,f.a);
}
