...
// [Mouse available] 3D morphing between 2 distance fields is just a "mix" between the distances of the 2 scenes :) (possible to extend it to colors of course).
...
void main() {
	...
	t=0.;
	for(int i=0;i<60;i++) 
		if (h>.0001 && t<4.7) t += h = .7*M(o + d * t);
		
	// Background colour change as teapot complementaries colours (using HSV)
	vec3 c = mix(hsv2rgb_smooth( .1+iGlobalTime*.02,.35,.4), 
			 	 hsv2rgb_smooth(-.1+iGlobalTime*.02,.35,.7), q.y);
	...
	if (h < .001) {
		...
		vec3 c1 = hsv2rgb_smooth(.5+iGlobalTime*.02, 1.,1.); 
		// Shading
