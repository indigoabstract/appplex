...
// left to right: - uniform noise - triangular noise - gaussianish noise - moar gaussianish noise :)
...
void main(void)
{
	...
	if ( uv.x < 1.0/4.0 )
	{
		o = n1rand( uv );
		if ( uv.y < 1.0 / 4.0 )
			o = 0.85 - 0.7 * histogram( 1, uv, vec2(0.0/4.0,1.0/4.0), 1.0/4.0, HIST_SCALE );
	}
	else if ( uv.x < 2.0 / 4.0 )
	{
		o = n2rand( uv );
		if ( uv.y < 1.0 / 4.0 )
			o = 0.85 - 0.7 * histogram( 2, uv, vec2(1.0/4.0,2.0/4.0), 1.0/4.0, HIST_SCALE );
	}
	else if ( uv.x < 3.0 / 4.0 )
	{
		o = n3rand( uv );
		if ( uv.y < 1.0 / 4.0 )
			o = 0.85 - 0.7 * histogram( 3, uv, vec2(2.0/4.0,3.0/4.0), 1.0/4.0, HIST_SCALE );
	}
	else
	{
		o = n4rand( uv );
		if ( uv.y < 1.0 / 4.0 )
			o = 0.85 - 0.7 * histogram( 4, uv, vec2(3.0/4.0,4.0/4.0), 1.0/4.0, HIST_SCALE );
	}
