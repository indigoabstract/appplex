// https://www.shadertoy.com/view/MssXWM
// simple newbie stuff that hopefully explains some shit in terms of actually implementing sphere-tracing/ray-marching
...
vec3 renderworld(vec2 uv){ // this is where we do everything
	...
	vec3 camPos = vec3(2.*sin(iGlobalTime),2.*cos(iGlobalTime),1.);
...
void main(void) // IS LIKE C(++), you start with the main
{
	...
	vec2 uv = (gl_FragCoord.xy / iResolution.xy - 0.5) // -.5 to centre it (is now in range [-.5,.5])
