...
void main( void )
{
	...
	vec3 euler = vec3(
		sin(iGlobalTime * 0.2) * 0.625,
		cos(iGlobalTime * 0.1) * 0.625,
		iGlobalTime * 0.1 + sin(iGlobalTime * 0.3) * 0.5);
	...
	q.y = 0.5 - asin(rd.y)/pi + 0.512 + 0.001*iGlobalTime;
	q *= 2.34;
	...
	float fSpeed = 0.; //c + a * b * cos(a * iGlobalTime);
