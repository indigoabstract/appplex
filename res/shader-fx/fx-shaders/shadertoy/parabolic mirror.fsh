// https://www.shadertoy.com/view/MssSDH
// reflection by a parabolic mirror (tune light position with mouse)
#define NB_RAY 80.
#define x0 .85
...
void main(void)
{
	...
	gl_FragColor = (v>0.) ? vec4(reflected,ray*(1.-reflected),0.,1.) : vec4(0.);

	// draw mirror
	gl_FragColor += vec4(smoothstep(.0001,0.,v*v));
}
