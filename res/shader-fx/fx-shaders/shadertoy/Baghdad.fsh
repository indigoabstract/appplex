// https://www.shadertoy.com/view/ldXGzH
float M=512.0,L=0.01,R=3.0;
vec3 l=vec3(0.577);
vec3 bl=vec3(0.5,0.7,1.0);

vec4 u = vec4(1.0, sin(iGlobalTime*0.1)*0.2+0.6, iGlobalTime*0.2, -iGlobalTime*2.0);
vec4 v = vec4(0.0, 1.0-(cos(iGlobalTime*0.1)*0.25+0.5), 1.0, 1.0-(cos(iGlobalTime*0.05)*0.5+0.5));
...
void main() {	
	...
	vec3 cp = vec3(sin(iGlobalTime*0.2)*5.0, cos(iGlobalTime*0.2)*5.0, iGlobalTime*0.05);
