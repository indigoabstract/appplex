// https://www.shadertoy.com/view/XssXRH
// Effect from the Demokerho Invitation.
struct polygon{
	vec2 A, B, C;
};
...
void main(void)
{
	...
	coord*=1.0+rand(coord+iGlobalTime)/(pow(iGlobalTime,7.0)*3.0)-length(coord)*10.0/(pow(iGlobalTime*1.1,24.0));
	coord*=1.0+0.1*sin(iGlobalTime*0.1);
	...
	if(mod(coord.x+coord.y+iGlobalTime*0.1,0.2)<0.1){
	...
	for(float i=0.0; i<3.141*2.0; i+=3.141*0.2){
		float aikakerroin=iGlobalTime*0.6;
		vec2 A=vec2(0.0,0.0);
	...
	if(roundBox(coord, vec2(-0.3,0.0+vasenKulmakarva), vec2(0.08, 0.001) )<0.008)
		tulos.xyz-=vec3(0.7, 0.7, 0.7);
	tulos.xyz=tulos.xyz-vec3(min(max(-0.44+length(coord)*0.41,0.0),1.0))+vec3(0.06*rand(vec2(coord.x+coord.y,iGlobalTime*coord.y*coord.x)));
