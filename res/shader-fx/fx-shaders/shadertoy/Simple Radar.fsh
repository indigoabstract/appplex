...
void main(void)
{
	...
	float distanceToCenter = distance(center,gl_FragCoord.xy);	
	...
	//Rotate Line
	float angle = (-iGlobalTime*1.2);
	lineEnd = rotatePoint(center,angle,lineEnd);
	
	//Draw Line	
	float distPointToLine = LineToPointDistance2D(center,lineEnd,gl_FragCoord.xy);
	...
	//Draw Stela
	float angleStelaToApply = angleVec(normalize(lineEnd-center),normalize(gl_FragCoord.xy-center));
	if (angleStelaToApply<angleStela && distanceToCenter<radius-circleWitdh/2.0+1.0)
	{
		...
		float distToBlip = distance(gl_FragCoord.xy,blips[0]);//blips[0]);
