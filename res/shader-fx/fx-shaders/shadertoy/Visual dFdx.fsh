// https://www.shadertoy.com/view/lsXSzn
// Visually showing GLSL's partial derivative functions! Function is in red, derivative is in blue. On my computer, at extreme cases, the blue line displays as 2-pixel-wide dashes, revealing my GPU's fragment shader architecture.
...
void main(void)
{
	...
	// Color vertical grid
		...
		if (abs(uv.x) < 0.5)
			...
			gl_FragColor = vec4(0,0.2,0.1,1) * (mod(uv.x, 5.0) * 0.25 + 0.25);
	...
	// Color horizontal grid
		...
		if (abs(uv.y) < 0.5)
			...
			gl_FragColor = vec4(0,0.2,0.1,1) * (mod(uv.y, 5.0) * 0.25 + 0.25);
