// https://www.shadertoy.com/view/lsfGRB
...
void main(void)
{
	...
	vec2 u = (gl_FragCoord.xy)/max(iResolution.x,iResolution.y);
...
//	float t = (rand(gl_FragCoord.xy)*2.0-1.0)*spread;
	float t = (texture2D(iChannel0,u*4.0).x*2.0-1.0)*spread;
		c += trace(P,V,H, t);
//	}
	//c*=0.2;
