// https://www.shadertoy.com/view/MdsSWr

// iChannel2: kb

// M: toggles construction mesh L: toggles distance field / thin line G: show gradient I: show isolines T: manual tunings C: shows parts drawn with the costly algo H: show bounding boxes
// efficient distance to spline relying on the iterative construction of splines:
// analytical distance bounds are evaluated.
// Notes:
// - ambiguous cases are treated numerically, but overall cost is negligible since rarery used.
// - signed squared distances are used instead of signed distances, but it saves nothing
// - programming without recursivity or piles in GLSL ES is a pain :-)

#define POINTS 8  		 // number of control points
#define SUBDIV 4         // subdivision depth for the analytic method
#define sample 120. 	 // number of samples per spline for the costly method
#define  eps 3e-3     	 // mesh thickness
#define DIST2 1		 	 // use (signed) square distances
#define SORT  1          // presort possible closest splines (faster, but varying cost)
                              // seems there is occasionnaly in/out bug in this mode

bool SHOW_MESH, SHOW_LINE, SHOW_GRAD, SHOW_ISO, SHOW_COSTLY, SHOW_INSIDE;
...
#if !DIST2
float dist2seg2(vec2 pos, vec2 P0, vec2 P1) 
#else
...
#endif
{
	...
	if      (l <= 0.) return dist2(pos,P0);
	else if (l >= 1.) return dist2(pos,P1);

	// --- dist to seg = dist to line
	else if (SIGNED_DIST) 
	{ float D = cross(P0P1,P0Pos); return sign(D)*D*D/l2_01; } // D/sqrt(l2_01); }
	else return dist2(pos, P0+l*P0P1);
}
#if !DIST2
float dist2seg(vec2 pos, vec2 P0, vec2 P1) 
	{ float d = dist2seg2(pos,P0,P1); return sign(d)*sqrt(abs(d)); }
#endif
...
#define ssign(v) (((v)>=0.) ? 1 : -1)
	...
	int t = t0+t1+t2+t3; // sum of turn sign = +- 4, +-2 or 0.
	 ...
	 // if direct, makes it indirect: we want negative inside
	 // eval from sign of t0, but in the special case where shape = '<' in t0 
	FLIP = SIGNED_DIST && ( (((t==2)||(t==-2)) && (t0*t<0)) ? (t0<0) : (t0>0) );
	if (FLIP) { t0=-t0; t1=-t1; t2=-t2; t3=-t3; t=-t; } 
	...
	if ((t==4)||(t==-4)) // --- all same sign: P0P1P2P3 convex-> paths 0123, 30
	  ...
	  if (t==0)         // --- 'X' shape
		...
		if (t1*t<0)       // it's P1 : del it.  -> P0P2P2P3 -> paths 023, 30
	        ...
	        { d0 = amin( amin( d01, d12 ), d23);	d1 = d30; }
#else
		else if (t3*t<0)  // it's P3 : del it.  -> P0P1P2P2 -> paths 012, 20
			{ d0 =  amin( d01, d12 );	d1 = dist2seg(pos,P2,P0); }
		else              // it's P0 : del it.  -> P1P1P2P3 -> paths 123, 31
			{ d0 = amin(d12, d23);      d1 = dist2seg(pos,P3,P1); }
#endif
	  }
...
#define eps2 ( (DIST2==1) ? eps*eps : eps )
		if (SHOW_MESH && ( (d0mM.x<eps2)||(d3mM.x<eps2) ) )
...
void main(void)
{
	...
	if ((iMouse.z<=0.) && !keyToggle(64+20)) { // 'T'
	...
	SHOW_INSIDE = keyToggle(64+2);  // 'B'
	
	
	// --- dist to spline 
		
#if SORT // precompute distance bounds for each spline + the min of max bounds.
	float dm[POINTS];
	float dM=1e5;
	P3=Pc, T3=Tc;
	for (float i=1.; i<float(POINTS); i++) 
	{
		...
		vec2 dmMi = dist2quadri(uv, P0,P0+T0,P3-T3,P3);
		dm[int(i)] = dmMi.x;      // negative if inside the bouding quadrilateral
		dM = min(dM,abs(dmMi.y)); // search min of max bounds
	}
#endif
	...
	{
		P0 = P3; T0 = T3; P3 = P(i); T3 = P(i+.5);
#if SORT 		// consider only the closest splines 
		float d = dm[int(i)];
		if (SHOW_INSIDE &&(d<0.)) gl_FragColor.b = 1.;	   
		if (d > dM+eps) continue;
#endif
		...
		if (dmMi.x==8.) // display ambiguous parts
			{ gl_FragColor=vec4(.18,0.,0.,0.); return; } 
		dmM = min (dmM, dmMi);
	}
#if DIST2
	dmM = sqrt(abs(dmM));
