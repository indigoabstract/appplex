...
void main(void) {
	...
	if (gl_FragCoord.y > iResolution.y / 2.0) {
		if (gl_FragCoord.x < iResolution.x / 2.0) {
			// 1D
			float coord = gl_FragCoord.x * 0.05 + iGlobalTime * 5.0 - 10.0;
			float height = NOISE(coord) * iResolution.y / 2.0;
			v = clamp((height - gl_FragCoord.y + iResolution.y / 2.0) / (iResolution.y * 0.02), 0.0, 1.0);
		} else if (gl_FragCoord.x < iResolution.x / 2.0 + 1.0) {
			gl_FragColor.rgb = vec3(1.0);
			return;
		} else {
			// 2D
			vec2 coord = gl_FragCoord.xy * 0.1 - vec2(iGlobalTime * 5.0, iResolution.y / 2.0);
			v = NOISE(coord);
		}
	} else if (gl_FragCoord.y > iResolution.y / 2.0 - 1.0) {
		gl_FragColor.rgb = vec3(1.0);
		return;
	} else {
		// Ray-sphere
		...
		vec3 w = normalize(vec3(gl_FragCoord.xy - iResolution.xy / vec2(2.0, 4.0), 
	...
	gl_FragColor.rgb = pow(v, 0.35) * 1.3 * normalize(vec3(0.5, gl_FragCoord.xy / iResolution.xy)) + vec3(v * 0.25);
}
