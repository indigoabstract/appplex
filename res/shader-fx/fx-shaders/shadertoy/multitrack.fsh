// https://www.shadertoy.com/view/MdBSRD
...
{
    return smoothstep(-abs(smooth), 0.0, 1.-abs((r - distance(coord, p)) / r));
}
...
void main(void)
{
    ...
    for(int i = 0; i < 16; ++i)
    {
        ...
        float env = adsr(vec4(0.06, 0.4, 0.2, 0.2), pt);
        
        float am;
        am = clamp(0.0, 1.0, env) * 0.8;
