...
void main(void)
{
	...
	vec3 colA = texture2D_smooth( iChannel0, uv, vec2(textureResolution) ).xyz;

    //---------------------------------------------	
	// my own filtering 
    //---------------------------------------------	
	//vec3 colB = texture2D_smooth( iChannel0, uv, vec2(textureResolution) ).xyz;
	vec3 colB = texture2D_bicubic( iChannel0, uv, vec2(textureResolution) ).xyz;	
	
    //---------------------------------------------	
    // mix between the two colors
    //---------------------------------------------	
	vec3 col = mix( colA, colB, smoothstep( -0.3, 0.3, sin(1.0*p.x + 3.1416*iGlobalTime) ) );
