...
+ (video_fb*) cGFrameBufferWithBppDimensions:(NSInteger)bitsPerPixel
                                            width:(NSInteger)width
                                           height:(NSInteger)height
{
  video_fb *obj = [[video_fb alloc] initWithBppDimensions:bitsPerPixel width:width height:height];
  return obj;
}
...
- (void) copyPixels:(video_fb *)anotherFrameBuffer
{
...
- (void) memcopyPixels:(video_fb *)anotherFrameBuffer
{
  [self doneZeroCopyPixels];
...
{
  return [NSString stringWithFormat:@"video_fb %p, pixels %p, %d x %d, %d BPP, isLocked %d", self, self.pixels,
...
void CGFrameBufferProviderReleaseData (void *info, const void *data, size_t size) {
	...
	NSLog(@"CGFrameBufferProviderReleaseData() called");
  	...
  	video_fb *cgBuffer;
  cgBuffer = (__bridge video_fb *) info;
