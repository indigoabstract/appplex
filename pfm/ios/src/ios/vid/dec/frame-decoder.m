...
@property (nonatomic, retain) AVAssetReaderOutput *aVAssetAudioReaderOutput;

@property (nonatomic, retain) video_fb *currentFrameBuffer;

// This is the last video_frame object returned via a call to advanceToFrame

@property (nonatomic, retain) video_frame *lastFrame;

@end


@implementation frame_decoder
...
@synthesize aVAssetAudioReaderOutput = m_aVAssetAudioReaderOutput;
...
{
  frame_decoder *obj = [[frame_decoder alloc] init];
  obj->frameIndex = -1;
  obj.dropFrames = TRUE;
    obj->reading_error = false;
...
- (BOOL) setupAsset
{
  ...
  if (self.produceYUV420Buffers == FALSE) {
    ...
    NSArray *audioTracks = [avUrlAsset tracksWithMediaType:AVMediaTypeAudio];
    ...
    if (shouldRecordAudioTrack)
    {
        ...
        [m_aVAssetReader addOutput:readerAudioTrackOutput];
        self.aVAssetAudioReaderOutput = readerAudioTrackOutput;
    }
  ...
  float duration = (float)CMTimeGetSeconds(timeRange.duration);
    ...
    NSTimeInterval nominalFrameRate = videoTrack.nominalFrameRate;
    //float nominalFrameRate = 10;//videoTrack.nominalFrameRate;
  ...
  if (self.dropFrames == TRUE) {
    ...
    if (CMTIME_IS_INVALID(minFrameDuration)) {
      ...
      // the time divided into even duration intervals.
        ...
        //frameDuration = 0.1;//CMTimeGetSeconds(minFrameDuration);
      nominalFrameRate = 1.0 / frameDuration;
    }
  }
  
    self->m_nominalFrameRate = (NSTimeInterval)nominalFrameRate;
  ...
  [m_aVAssetReader addOutput:aVAssetReaderOutput];
...
- (void) blockingDecodeVerifySize:(video_fb*)frameBuffer
{
...
- (FrameReadStatus) blockingDecodeReadFrame:(video_fb**)frameBufferPtr
...
//#endif // LOGGING
  ...
  else if ([aVAssetReader status] == AVAssetReaderStatusReading && !reading_error)
  {
    AVAssetReaderStatus status = aVAssetReader.status;
    NSError *error = aVAssetReader.error;
...
// Render sample buffer into flat video_fb. We can't read the samples
...
- (BOOL) renderCMSampleBufferRefIntoFramebuffer:(CMSampleBufferRef)sampleBuffer frameBuffer:(video_fb**)frameBufferPtr
{
...
- (BOOL) renderCVYUVImageBufferRefIntoFramebuffer:(CVImageBufferRef)imageBuffer frameBuffer:(video_fb**)frameBufferPtr
{
...
- (BOOL) renderCVBGRAImageBufferRefIntoFramebuffer:(CVImageBufferRef)imageBuffer frameBuffer:(video_fb**)frameBufferPtr
{
  video_fb *frameBuffer = *frameBufferPtr;
  ...
  if (frameBuffer == NULL) {
    frameBuffer = [[video_fb alloc] initWithBppDimensions:24 width:width height:height];
...
//  NSLog(@"advanceToFrame : current %d, new %d", (frameIndex + 1), newFrameIndex);
  ...
  if (!changeFrameData) {
    ...
    // When no change from previous frame is found, return a new video_frame object
    // but make sure to return the same image object as was returned in the last frame.
    
    retFrame = [video_frame vid_frame];
    NSAssert(retFrame, @"video_frame is nil");
    ...
    if (self.produceCoreVideoPixelBuffers) {
      ...
      // an video_frame object but there is no ref to a CoreVideo path. The optimal path
      ...
      video_fb *cgFrameBuffer = self.currentFrameBuffer;
    ...
    if (self.produceCoreVideoPixelBuffers) {
      ...
      // Return a CGImage wrapped in a video_frame
      video_fb *cgFrameBuffer = self.currentFrameBuffer;
      retFrame.cgFrameBuffer = cgFrameBuffer;
...
{
  video_frame *frame = [video_frame vid_frame];
...
// Gettter for nominalFrameRate property

- (NSTimeInterval) nominalFrameRate
{
    float nominalFrameRate = self->m_nominalFrameRate;
