...
public:
	...
	struct rt
	{
		...
		{
			shared_ptr<std::vector<T> > vect(new std::vector<T>(get_render_target_width() * get_render_target_height()));
		...
		template<typename T> static void get_render_target_pixels(shared_ptr<gfx_rt> irt, shared_ptr<std::vector<T> > ivect)
		{
			get_render_target_pixels_impl(irt, begin_ptr(ivect));
		}
	};
	...
	struct tex
	{
		static shared_ptr<gfx_tex> new_tex_2d(std::string iuni_tex_name);
		static shared_ptr<gfx_tex> new_tex_2d(std::string iuni_tex_name, int iwith, int iheight);
		static shared_ptr<gfx_tex> new_tex_2d(std::string iuni_tex_name, int iwith, int iheight, std::string iformat);
		static shared_ptr<gfx_tex> new_tex_2d(std::string iuni_tex_name, int iwith, int iheight, int iinternal_format, int iformat, int itype);
		static shared_ptr<gfx_tex> new_external_tex_2d(std::string iuni_tex_name, int itexture_id, int iwith, int iheight, int iinternal_format, int iformat, int itype);
