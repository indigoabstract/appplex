...
void gfx_vxo::render_mesh_impl(shared_ptr<gfx_camera> icamera)
{
	...
	for (std::vector<shared_ptr<vx_attribute> >::iterator it = vxi.vx_aux_attr_vect.begin(); it != vxi.vx_aux_attr_vect.end(); it++)
	{
		...
		if (loc_idx != -1)
		{
			...
			glVertexAttribPointer(loc_idx, at->get_component_count(), gl_type, normalized, vxi.aux_vertex_size, (const void*)offset_aux);// begin_ptr(aux_vertices_buffer) + offset_aux);
			glEnableVertexAttribArray(loc_idx);
		}
	...
	gfx_util::check_gfx_error();
	glDrawElements(method, indices_buffer.size(), GL_UNSIGNED_INT, 0);// begin_ptr(indices_buffer));
	gfx_util::check_gfx_error();
	...
	if (wireframe_mode)
	{
		...
		glDrawElements(GL_LINE_STRIP, indices_buffer.size(), GL_UNSIGNED_INT, 0);// begin_ptr(indices_buffer));
