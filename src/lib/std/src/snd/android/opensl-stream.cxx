...
#if defined MOD_SND && defined PLATFORM_ANDROID
...
#include <snd/android/opensl-stream.hxx>
...
#define LOGI(...) __android_log_print(ANDROID_LOG_INFO, "opensl_stream", __VA_ARGS__)
#define LOGW(...) __android_log_print(ANDROID_LOG_WARN, "opensl_stream", __VA_ARGS__)
...
static void updateIntervals(timespec *previousTime, double thresholdMillis, int *intervals, int *offset, int *previousIndex, int index)
{
	timespec t;
	...
	if (previousTime->tv_sec + previousTime->tv_nsec > 0)
	{
		...
		double dt = (t.tv_sec - previousTime->tv_sec) * 1e3 + (t.tv_nsec - previousTime->tv_nsec) * 1e-6;
...
static void recorderCallback(SLAndroidSimpleBufferQueueItf bq, void *context)
{
	...
	if (p->outputChannels)
	{
		...
		{
			updateIntervals(&p->inputTime, p->thresholdMillis, &p->inputIntervals, &p->inputOffset, &p->previousInputIndex, p->inputIndex);
		}
	}
	else
	{
		const short* input_buffer = p->inputBuffer + (p->inputIndex % p->inputBufferFrames) * p->inputChannels;
		p->callback(p->context, p->sampleRate, p->callbackBufferFrames, p->inputChannels, input_buffer, 0, NULL);
	}

	__sync_bool_compare_and_swap(&p->inputIndex, p->inputIndex, nextIndex(p->inputIndex, p->callbackBufferFrames));

	const short* input_buffer = p->inputBuffer + (p->inputIndex % p->inputBufferFrames) * p->inputChannels;

	(*bq)->Enqueue(bq, input_buffer, p->callbackBufferFrames * p->inputChannels * sizeof(short));
}
...
static void playerCallback(SLAndroidSimpleBufferQueueItf bq, void *context)
{
	...
	if (p->inputChannels)
	{
		...
		{
			updateIntervals(&p->outputTime, p->thresholdMillis, &p->outputIntervals, &p->outputOffset, &p->previousOutputIndex, p->outputIndex);
		}

		if (p->readIndex < 0 && p->outputIntervals == STARTUP_INTERVALS && __sync_fetch_and_or(&p->inputIntervals, 0) == STARTUP_INTERVALS)
		{
			int offset = p->inputOffset + p->outputOffset + OUTPUT_BUFFERS * p->callbackBufferFrames;

			p->readIndex = __sync_fetch_and_or(&p->inputIndex, 0) - offset;
		}
	}

	short *currentOutputBuffer = p->outputBuffer + (p->outputIndex % p->outputBufferFrames) * p->outputChannels;

	memset(currentOutputBuffer, 0, p->callbackBufferFrames * p->outputChannels * sizeof(short));

	if (p->readIndex >= 0)
	{  // Synthesize audio with input if available.
		...
		if (margin < p->lowestMargin && -p->inputBufferFrames < margin)
		{
			p->lowestMargin = margin;
		}

		const short* input_buffer = p->inputBuffer + (p->readIndex % p->inputBufferFrames) * p->inputChannels;

		p->callback(p->context, p->sampleRate, p->callbackBufferFrames, p->inputChannels, input_buffer, p->outputChannels, currentOutputBuffer);
		p->readIndex = nextIndex(p->readIndex, p->callbackBufferFrames);
	}
	else
	{  // Synthesize audio with empty input when input is not yet availabe.
		p->callback(p->context, p->sampleRate, p->callbackBufferFrames, p->inputChannels, p->dummyBuffer, p->outputChannels, currentOutputBuffer);
	}
...
static SLresult openSLRecOpen(OPENSL_STREAM *p, SLuint32 sr)
{
	...
	SLresult result = (*p->engineEngine)->CreateAudioRecorder(p->engineEngine, &p->recorderObject, &audioSrc, &audioSnk, 1, id, req);
	...
	result = (*p->recorderBufferQueue)->RegisterCallback(p->recorderBufferQueue, recorderCallback, p);
...
static SLresult openSLPlayOpen(OPENSL_STREAM *p, SLuint32 sr)
{
	...
	SLresult result = (*p->engineEngine)->CreateOutputMix(p->engineEngine, &p->outputMixObject, 1, mixIds, mixReq);
	...
	result = (*p->engineEngine)->CreateAudioPlayer(p->engineEngine, &p->playerObject, &audioSrc, &audioSnk, 1, playIds, playRec);
	...
	result = (*p->playerObject)->GetInterface(p->playerObject, SL_IID_PLAY, &p->playerPlay);

	if (result != SL_RESULT_SUCCESS) return result;

	result = (*p->playerObject)->GetInterface(p->playerObject, SL_IID_ANDROIDSIMPLEBUFFERQUEUE, &p->playerBufferQueue);
...
OPENSL_STREAM *opensl_open(int sampleRate, int inChans, int outChans, int callbackBufferFrames, opensl_process_t proc, void *context)
{
	if (!proc)
	{
		return NULL;
	...
	p->inputBufferFrames = (sampleRate / callbackBufferFrames / 4) * callbackBufferFrames;
	...
	if (false)//inChans)
	{
		...
		p->dummyBuffer = (short *)calloc(callbackBufferFrames * inChans, sizeof(short));

		if (p->inputBuffer == nullptr || p->dummyBuffer == nullptr)
		{
			opensl_close(p);
...
#endif // defined MOD_SND && defined PLATFORM_ANDROID
