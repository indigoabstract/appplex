...
template <class Arguments> 
inline boost::shared_array<wchar_t> collection_to_win32_cmdlinew(const Arguments &args) 
{ 
	typedef std::vector<std::wstring> arguments_t; 
	arguments_t args2; 
	typename Arguments::size_type i = 0; 
	std::size_t size = 0; 
	for (typename Arguments::const_iterator it = args.begin(); it != args.end(); ++it) 
	...
	boost::shared_array<wchar_t> cmdline(new wchar_t[size]); 
	cmdline.get()[0] = L'\0'; 
	for (arguments_t::size_type i = 0; i < args.size(); ++i) 
#if defined(__MINGW32__)
		::wcsncat(cmdline.get(), args2[i].c_str(), args2[i].size()); 
#else 
		::wcscat_s(cmdline.get(), size, args2[i].c_str());
#endif 
...
template <class Executable, class Arguments> 
inline PROCESS_INFORMATION win32_startw(const Executable &exe, const Arguments &args, const environment &env, stream_info &infoin, stream_info &infoout, stream_info &infoerr, const win32_setupw &setup) 
{ 
	file_handle chin, chout, cherr; 
	...
	case stream_info::use_file: 
		{ 
			HANDLE h = ::CreateFileA(infoin.file_.c_str(), GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL); 
			if (h == INVALID_HANDLE_VALUE) 
				boost::throw_exception(boost::system::system_error(boost::system::error_code(::GetLastError(), boost::system::get_system_category()), "boost::process::detail::win32_startw: CreateFile failed")); 
			chin = file_handle(h); 
			break; 
		} 
	case stream_info::use_handle: 
	...
	boost::shared_array<wchar_t> cmdline = collection_to_win32_cmdlinew(args); 

	boost::scoped_array<wchar_t> executable(new wchar_t[exe.size() + 1]); 
#if defined(__MINGW32__)
	::wcscpy(executable.get(), exe.c_str()); 
#else 
	::wcscpy_s(executable.get(), exe.size() + 1, exe.c_str());
#endif 

	boost::scoped_array<wchar_t> workdir(new wchar_t[setup.work_directory.size() + 1]); 
#if defined(__MINGW32__)
	::wcscpy(workdir.get(), setup.work_directory.c_str()); 
#else 
	::wcscpy_s(workdir.get(), setup.work_directory.size() + 1, setup.work_directory.c_str());
#endif 

	boost::shared_array<char> envstrs = environment_to_win32_strings(env); 

	if (!::CreateProcessW(executable.get(), cmdline.get(), NULL, NULL, TRUE, 0, envstrs.get(), workdir.get(), si.get(), &pi)) 
		boost::throw_exception(boost::system::system_error(boost::system::error_code(::GetLastError(), boost::system::get_system_category()), "boost::process::detail::win32_startw: CreateProcess failed")); 
