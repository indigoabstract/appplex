...
#if defined(BOOST_WINDOWS_API)
template <class Executable, class Arguments, class Context>
inline child launchw(const Executable &exe, const Arguments &args, const Context &ctx) 
{ 
	detail::file_handle fhstdin, fhstdout, fhstderr; 
	...
	detail::win32_setupw s; 
	s.work_directory = ctx.work_directory; 
	s.startupinfo = &si; 

	PROCESS_INFORMATION pi = detail::win32_startw(exe, args, ctx.environment, behin, behout, beherr, s); 

	if (!::CloseHandle(pi.hThread)) 
		boost::throw_exception(boost::system::system_error(boost::system::error_code(::GetLastError(), boost::system::get_system_category()), "boost::process::launchw: CloseHandle failed")); 
...
#if defined(BOOST_WINDOWS_API) 
template <class Context> 
inline child launch_shellw(const std::wstring &command, const Context &ctx) 
{ 
	std::wstring exe; 
	...
	if (!size) 
		boost::throw_exception(boost::system::system_error(boost::system::error_code(::GetLastError(), boost::system::get_system_category()), "boost::process::launch_shellw: GetWindowsDirectory failed")); 
	BOOST_ASSERT(size < MAX_PATH); 

	exe = std::wstring(sysdir) + (sysdir[size - 1] != L'\\' ? L"\\cmd.exe" : L"cmd.exe"); 
	args.push_back(L"cmd"); 
	args.push_back(L"/c"); 
	args.push_back(command); 

	return launchw(exe, args, ctx); 
} 
#endif 
