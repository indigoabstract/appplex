...
public:
   ...
   mws_sp<mws_media_info> get_media_info()
   {
      return mi;
   }
   ...
   void update(mws_sp<gfx_camera> i_cam)
   {
      ...
      mws_sp<gfx_rt> current_rt = gfx::i()->rt.get_current_render_target();
      mws_sp<gfx_shader> current_program = gfx::i()->shader.get_current_program();
   ...
   void set_listener(mws_sp<mws_vdec_listener> i_listener)
   {
      listener = i_listener;
   }

   mws_sp<mws_media_info> mi;

private:
   friend class ffmpeg_media_info;

   int decode_frame(mws_sp<gfx_camera> i_cam)
   {
      int ret = 0;
   ...
   mws_sp<gfx_tex> y_tex;
   mws_sp<gfx_tex> u_tex;
   mws_sp<gfx_tex> v_tex;
   ...
   std::vector<char> ut;
   std::vector<char> vt;
   mws_sp<gfx_quad_2d> q2d;
   mws_sp<gfx_shader> conv_yuv420sp_2_rgb;
   mws_sp<gfx_rt> rt;
...
{
   mws_sp<vdec_ffmpeg_impl> i = impl.lock();
...
mws_sp<gfx_tex> ffmpeg_media_info::get_current_frame()
{
...
{
   impl = mws_sp<vdec_ffmpeg_impl>(new vdec_ffmpeg_impl());
   impl->mi = mws_sp<mws_media_info>(new ffmpeg_media_info(impl));
}
...
mws_sp<mws_media_info> vdec_ffmpeg::get_media_info()
{
...
void vdec_ffmpeg::update(mws_sp<gfx_camera> i_cam)
{
   switch (get_state())
   {
   case mws_vdec_state::st_stopped:
...
void vdec_ffmpeg::set_listener(mws_sp<mws_vdec_listener> listener)
{
