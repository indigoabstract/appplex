...
void DoLighting(inout vec3 mat, in vec3 pos, in vec3 normal, in vec3 eyeDir, in float dis)
{
	...
	if (h > 0.0)
	{
		...
		float specAmount = pow( max(dot(R, normalize(eyeDir)), 0.0), 5.0)*specular;
...
{
    float t = 10.0 + Hash12(uv*3.0)* 13.0;
	float oldT = 0.0;
	float delta = 0.0;
	bool fin = false;
	vec2 distances;
	...
	for( int j=0; j< 105; j++ )
	{
		...
		t += delta;
   		...
   		h = MapClouds(p);
		shade.y = max(-h, 0.0); 
		shade.x = max(p.y-120.0, 0.0)*.00004;
    ...
    cloud.x = min(pow(max(cloud.x, 0.05), .3), 1.0);
...
vec3 CameraPath( float t )
{
	...
	t =((iGlobalTime*2.0)+m+1820.0)*.004 + t;
    vec2 p = 1500.0*vec2( sin(4.5*t), cos(3.5*t) );
	return vec3(3750.0+p.x, 0.6, -2200.0+p.y);
}
...
void main(void)
{
    ...
    float bri = pow(max(dot(rd, sunLight), 0.0), 6.0)*4.0+.2;
    bri = ((cloud.y)) * bri;
    col = mix(col, vec3(min(vec3(.6)*cloud.x + bri *.5 * sunColour, 1.0)), min(cloud.y*(bri+1.0), 1.0));
	...
	gl_FragColor=vec4(col*smoothstep(0.0, 2.0, iGlobalTime) ,1.0);
}
