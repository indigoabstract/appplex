...
void main()
{
    ...
    // Create the background dots.
    {
        ...
        gl_FragColor.rgb = 10.3*vec3(0.01,0.01,0.02)*
    ...
    if(t<cull_dist)
    {
        ...
        gl_FragColor.rgb=dc*mix(vec3(0.04),vec3(0.2,0.2,0.22),max(0.0,dot(rn,normalize(vec3(3.0,1.0,1.0)))));
        ...
        for(int j=-1;j<=1;j+=1)
        {
            ...
            {
                gl_FragColor.rgb+=lightscale*2.0*vec3(0.3,0.7,1.0)*mix(0.1,1.0,pow(1.0-dot(normalize(rp-lp),-rd),1.0));
            }
            else
            {
                vec3 ld=normalize(lp-rp);
                gl_FragColor.rgb+=lightscale*60.0*dc*vec3(0.3,0.7,1.0)*vec3(max(0.0,dot(ld,rn)))*pow(max(0.0,1.0-distance(rp,lp)*0.2),4.0);
            }
        }
    }
    ...
    // Create the wires.
    {
        ...
        gl_FragColor.rgb*=mix(0.0,1.0,mask);
        // Add some fake lighting.
        gl_FragColor.rgb+=dust;
    }

    gl_FragColor.rgb+=glow+0.6*vec3(0.3,0.7,1.0)*mix(vec3(0.01),vec3(0.0),min(1.0,abs(coord.x)));
    gl_FragColor.rgb=tonemap(gl_FragColor.rgb*10.0)+noise(gl_FragCoord.xy)/255.0;
}
