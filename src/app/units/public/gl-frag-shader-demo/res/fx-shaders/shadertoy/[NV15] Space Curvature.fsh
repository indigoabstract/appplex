...
float dSphere( in vec3 ro, in vec3 rd, in vec4 sph )
{
	vec3 oc = ro - sph.xyz;
    float b = dot( oc, rd );
...
vec3 iNormal( in vec3 pos, in vec4 sph )
{
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    //sph1 = vec4( 5.0*sin(iGlobalTime), 0.0, 0.0, 1.0 );
    ...
    float gl = 1.0;//0.5 + 0.5*sin(2.0*iGlobalTime);

    col = background( rd, lig );
    ...
    if( t>0.0 )
    {
        ...
        nor = iNormal( pos, sph1 );
            ...
            mat = mix( mat, vec3(0.93), clouds*0.15 );
    ...
    if( t2<tmax )
    {
            ...
            float fo = 1.0*exp( -0.05*t*t );//*(1.0-smoothstep(10.0,15.0,t));

            float ph = floor( pos.x*4.0  + 0.5);
            float ra = fract(sin(ph)*11113.1);
            float en = smoothstep( 0.98,1.0,sin(ra*10.0 + 1.0*pos.z + 2.0*iGlobalTime));
            en *= 0.0;
            
            vec3 wir = vec3( 0.0 );
            wir += 1.0*exp(-12.0*abs(scp.x)*(1.0-0.5*en)) * (1.0 + en);
            wir += 1.0*exp(-12.0*abs(scp.y));
            wir += 0.5*exp( -4.0*abs(scp.x)*(1.0-0.2*en)) * (1.0 + en);
    ...
    float d = dSphere( ro, rd, sph1 );
    vec3 glo = vec3(0.0);
    glo += vec3(0.6,0.7,1.0)*0.3*exp(-2.0*abs(d))*step(0.0,d)*gl;
