...
{
public:
   static mws_sp<voronoi_gen> nwi(mws_sp<mws_vrn_data> idiag_data)
   {
   ...
   void clear_points()
   {
      vx.clear();
      vy.clear();
      diag_data_state = mws_vrn_diag_data_change_type::e_diag_no_change;
   }
   ...
   void random_points()
   {
      ...
      float hw = diag_data->info.diag_width;
      float hh = diag_data->info.diag_height;
      float x_inf = 50;
      float x_sup = hw - 50;
      float y_inf = 50;
      float y_sup = hh - 50;
...
private:
   ...
   void init(mws_sp<mws_vrn_data> idiag_data)
   {
...
public:
   ...
   static mws_sp<mws_vrn_geom> nwi(mws_sp<mws_vrn_data> idiag_data, mws_sp<gfx_camera> i_cam)
   {
      mws_sp<mws_vrn_geom> inst(new mws_vrn_geom(idiag_data));
   ...
   void set_line_geometry(mws_vrn_nexus_pair_vect& iline_points, mws_sp<gfx_vxo> imesh)
   {
      ...
      {
         mws_vrn_nexus_pair& p = iline_points.vect[k];
   ...
   void set_points_geometry(mws_vrn_pos_vect& ipoint_list, mws_sp<gfx_vxo> imesh)
   {
   ...
   void set_triangle_geometry(mws_vrn_pos_vect& ipoint_list, const std::vector<int>& ipoint_count_list, mws_sp<gfx_vxo> imesh)
   {
   ...
   mws_sp<mws_vrn_data> diag_data;
   mws_sp<gfx_node> voronoi_landscape_mesh;
...
private:
   mws_vrn_geom(mws_sp<mws_vrn_data> idiag_data) : gfx_node(gfx::i())
   {
      diag_data = idiag_data;
   }
   ...
   void init(mws_sp<gfx_camera> i_cam)
   {
      ...
      voronoi_landscape_mesh = mws_sp<gfx_node>(new gfx_node(gfx::i()));
      ...
      (*nexus_pairs_mesh)["u_v4_color"] = glm::vec4(0.f, 0.f, 1.f, 1.f);
      ...
      get_mws_sp()->attach(voronoi_landscape_mesh);
   }
...
{
public:
   static mws_sp<mws_vrn_main> nwi(int i_diag_width, int i_diag_height, mws_sp<gfx_camera> i_cam)
   {
   ...
   void move_kernel_to(int i_idx, float i_x, float i_y);
   void insert_kernel_at(float i_x, float i_y);
   void remove_kernel(int i_idx);
...
private:
   ...
   void init(int i_diag_width, int i_diag_height, mws_sp<gfx_camera> i_cam)
   {
      ...
      vgeom = mws_vrn_geom::nwi(diag_data, i_cam);
   }
};
