...
shared_ptr<vx_attribute> gfx_util::parse_attribute(std::string iattribute)
{
	...
	a = shared_ptr<vx_attribute>(new vx_attribute(iattribute, it->second));

	return a;
}

std::vector<shared_ptr<vx_attribute> > gfx_util::parse_attribute_list(std::string iattr_list)
{
	std::vector<shared_ptr<vx_attribute> > v;

	int current_pos = 0;

	while (current_pos < iattr_list.length())
	{
...
shared_ptr<gfx_uniform> gfx_util::parse_uniform(std::string iuniform)
{
	...
	v = shared_ptr<gfx_uniform>(new gfx_uniform(iuniform, it->second));
...
void gfx_util::draw_tex(shared_ptr<gfx_tex> itex, float itx, float ity, float iw, float ih)
{
	...
	if (tex_dsp)
	{
		...
		shared_ptr<gfx_tex> u_s2d_tex = gfx::tex::new_external_tex_2d("u_s2d_tex", itex->get_texture_gl_id(), iw, ih, GL_RGBA, GL_RGBA, GL_UNSIGNED_BYTE);
...
{
	return floor(log2(std::max(iwidth, iheight))) + 1;
}
