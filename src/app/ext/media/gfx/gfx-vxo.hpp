	...
	vx_info(std::string ivx_attr_list)
	{
      ...
      uses_tangent_basis = false;
			...
			// calc tangents and bitangents
		{
			...
			for(std::vector<shared_ptr<vx_attribute> >::iterator it = vx_aux_attr_vect.begin(); it != vx_aux_attr_vect.end(); it++)
			{
...
{
public:
	gfx_vxo(vx_info ivxi, bool iis_submesh = false);
	virtual ~gfx_vxo();
...
protected:
	...
	virtual void render_mesh_impl(shared_ptr<gfx_camera> icamera);
	...
	shared_ptr<gfx_material> material;
