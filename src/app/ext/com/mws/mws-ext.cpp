...
{
	shared_ptr<mws_page_tab> mws_root = get_view()->get_root();
...
mws_list_menu_page::mws_list_menu_page(shared_ptr<mws_page_tab> iparent) : mws_page(iparent)
{
}

shared_ptr<mws_list_menu_page> mws_list_menu_page::new_instance(shared_ptr<mws_page_tab> iparent, string iid)
{
	shared_ptr<mws_list_menu_page> u = static_pointer_cast<mws_list_menu_page>(new_shared_instance(new mws_list_menu_page(iparent)));
	u->set_id(iid);
