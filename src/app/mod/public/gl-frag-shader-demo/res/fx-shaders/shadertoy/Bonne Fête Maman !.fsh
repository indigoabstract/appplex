// https://www.shadertoy.com/view/lsXXD4
...
vec4 flower(vec2 p, float n, float seed) {
	...
	float t = atan(v.y, v.x)+(rand(c)*2.0-1.0)*2.0*PI+iGlobalTime*rot;
...
vec3 backgroundColor(vec2 p) {
	...
	for (float i = 0.0 ; i < 5.5 ; i++) {
		vec2 pp = p;
		pp *= rot(.05*sin(2.0*iGlobalTime+2.0*PI*rand(vec2(i,1.0))));
		pp.x += iGlobalTime*(rand(vec2(i,2.0))*2.0-1.0)*(i+3.0)*.12;
		pp.y += sin(iGlobalTime+2.0*PI*rand(vec2(i,3.0)))*.1;
...
float heartFormula(vec2 p, bool time) {
	// heartbeat
	if (time) {
		float beatValue = beat(fract(0.824*iGlobalTime))*0.1;
...
float opening(vec2 p) {
	float mult = max(0.0, 5.0-iGlobalTime*1.5);
	p *= 3.0*mult*mult*mult;
	p *= rot(sin(iGlobalTime*6.0)*.2);
...
void main(void) {
	...
	float mult = 3.0+4.0*beat(min(1.0, 0.09*iGlobalTime));
	uv *= mult;
