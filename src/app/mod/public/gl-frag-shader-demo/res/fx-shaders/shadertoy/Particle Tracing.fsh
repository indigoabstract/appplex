...
void main(void)
{
	...
	cam_origin = rotateX(iGlobalTime * 0.3,
						 rotateY(iGlobalTime * 0.5, vec3(0.0, 0.0, -10.0 + 5.0 * cos(iGlobalTime * 0.1))));
	
	// calculate the rotation matrix
	vec3 cam_w = normalize(vec3(cos(iGlobalTime) * 10.0, 0.0, 0.0) - cam_origin);
	...
	gl_FragColor.rgb = sqrt(gl_FragColor.rgb * 0.8);
}
