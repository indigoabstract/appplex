...
void main(void)
{
	...
	uv.x += iGlobalTime * 0.03;	// scroll left with time
	vec3 finalColor = texture2D(iChannel1, uv * vec2(1.0, -1.0)).xyz;	// background texture
	finalColor = finalColor * finalColor;	// gamma correct

	//finalColor += ThickLine(uv, vec2(0.8, 0.2), vec2(0.8, 0.8), 8.0) * vec3(0.0, 10.0, 10.0);
	vec4 noise = texture2D(iChannel0, uv*3.0);
