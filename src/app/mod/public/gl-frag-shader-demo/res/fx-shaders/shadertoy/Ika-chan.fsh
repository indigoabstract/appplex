// https://www.shadertoy.com/view/lssGRn// by @301z
...
vec2 position() {
  return vec2((gl_FragCoord.x - iResolution.x), gl_FragCoord.y) / iResolution.y;
}
...
vec3 filter(vec3 color) {
  return color * 0.5 * (2.0 - gl_FragCoord.x / iResolution.x + gl_FragCoord.y / iResolution.y);
}
