...
// The castle fly-by sequence from the original Unreal, released in 1998.
...
void main()
{
    ...
    if(max(absrp.x,max(absrp.y,absrp.z))>6000.0)
    {
        ...
        gl_FragColor.rgb=mix(vec3(0.0),vec3(0.3,0.3,0.4),0.5+0.5*rd.y)*0.75;
        gl_FragColor.rgb+=sun;
        gl_FragColor.rgb=mix(gl_FragColor.rgb,vec3(1.0),(smoothNoise2(cloudtc*4.0)*0.25+
    ...
    // Render the first flag.
    {
        ...
        if(abs(frp.x)<1.0 && abs(frp.y)<0.6)
        {
            ...
            gl_FragColor.rgb=mix(gl_FragColor.rgb,fl.rgb,fl.a*step(ft,t)*step(0.0,ft));
        }
    }
    ...
    // Apply some fog.
    gl_FragColor.rgb=mix(vec3(0.15,0.15,0.18)*4.2,gl_FragColor.rgb,exp(-t*5e-6));
    ...
    // Render the torch flames and their reflections.
    {
        ...
        if(abs(frp.x)<0.8 && abs(frp.y)<1.0)
        {
            ...
            gl_FragColor.rgb=mix(gl_FragColor.rgb,vec3(0.0),clamp(dot(ff,vec3(1.0/3.0))*2.0,0.0,1.0));
            gl_FragColor.rgb+=ff;
        }
    }


    gl_FragColor.rgb+=vec3(1.0,0.6,0.3)*(1.0-smoothstep(0.0,0.5,abs(mod(time*0.5,29.0)-1.5)))*0.12;

    gl_FragColor.rgb+=sun*0.28;
