// https://www.shadertoy.com/view/lsXXz7
...
void main(void)
{
	...
	// Locate a tile
	Tile_t tile = tile_Locate(gl_FragCoord.xy, TILE_TRI, STYLE_SEAMLESS, size);	

	// Convert coordinate to a relative position in the tile
	vec2 uv = tile_CalcRelPositionUV(tile, gl_FragCoord.xy, 0.0);
