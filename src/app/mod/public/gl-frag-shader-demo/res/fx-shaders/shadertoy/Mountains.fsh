...
float Hash( float n )
{
	return  texture2D( iChannel0, vec2(n*.27, n*.26), -100.0 ).x;
}

//--------------------------------------------------------------------------
vec2 Hash2(float n)
{
	return texture2D( iChannel0, vec2(n*.077331, n*.066927), -100.0 ).xz;
}
...
float Trees(vec2 p)
{
	...
	return max(.8 * Noise(p)-(length(v2)), 0.0) * treeLine;
}
#endif
