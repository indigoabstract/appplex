...
vec4 MapGrassID( in vec3 pos, out float which)
{
	...
	return vec4(ret.xyz, .9-fract(which*382.321)*.15 + pos.y * .2 * smoothstep(6.5, 10.0, pos.y)-ret.w);// - ret.w-sin(which*123.0)*.5);
}
...
void main( void )
{
	...
	col 		= mix(col, GetClouds(vec3(.5), ro, rd),	pow(abs(rd.y), .5));
    vec3 background = col;
