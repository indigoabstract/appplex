...
// Importance of motion blur under three different frame rates (60, 30 and 15 FPS -- Shadertoy must be running at full 60 FPS for correct blurring). You can click on the shader to hide the jittering or the blurred circles.
...
void main(void)
{
	...
	vec2 coord = mod(gl_FragCoord.xy, resol);
	float view = floor(gl_FragCoord.x / resol.x);
