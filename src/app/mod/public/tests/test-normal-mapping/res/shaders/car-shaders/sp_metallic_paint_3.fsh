...
void main()
{
	vec3 v3_base_color = u_v4_paint_front_color.rgb;
	...
	v3_flake_nrm_ns.rg *= 0.75;
	v3_flake_nrm_ns.b *= 0.75;
	...
	v1_refl_offset = fract(v1_refl_offset * 0.317);
	float fga = v1_refl_offset;
	v1_refl_offset = smoothstep(0.0, 0.9, v1_refl_offset * v1_intensity * 9.);
	
	vec3 v3_gloss_color = mix(vec3(1.0), vec3(0.0), v1_refl_offset);
	v3_gloss_color += v4_refl_color.rgb * pow(3., v1_intensity);
	v3_spec_color += v3_gloss_color;

	float v1_mix_fact = pow(1.0 + v3_dot_view_normal, 555.0);
	...
	vec3 v3_c2 = /*pow(v1_fresnel_fact1, u_v1_fresnel_exponent * 100.0) * */v3_p_norm1_ns.rgb;// * v1_flake_limit;
	//v3_c2 += pow(v1_fresnel_fact1, u_v1_fresnel_exponent * 100.0) * v3_p_norm1_ns.rgb * v1_flake_limit;
	
	vec3 v3_frag_color = mix(v3_c1, v3_spec_color, v1_mix_fact) + v3_c2 * 0.25 + v3_base_color * v1_fresnel_fact1 * 0.2;
	v3_frag_color *= v_v4_color.rgb * 1.25;

	gl_FragColor = vec4(v3_frag_color, 1.0);
	//gl_FragColor = vec4(abs(v3_normal_ws), 1.0);
}
