...
// Another fake lighting trick with distance fields.
...
void main()
{
	...
	if ( Toggle(kLensFX) )
	{
		...
		col += LensFlare( ray, lightCol1, lightDir1, smoothstep(-.04,.1,s1), step(t,.0) );
		col += LensFlare( ray, lightCol2*lightIntensity2, lightDir2, smoothstep(-.04,.1,s2), step(t,.0) );
