...
// variation on https://www.shadertoy.com/view/Xs23D3 mouse.x: zoom S: mouse.x controls lacunarity instead E: mouse.y = exponent (=vicinity) N+B: noise type: 00: smooth 10: abs 11: 1-abs 10:1/n T: add or mul C: toggles colors G: toggles galaxy
// variations on noise 
...
bool display(vec2 pos, float scale, float offset, int number, int dot) { // dot: draw separator
	...
	gl_FragColor += seg*vec4(0.,.5,1.,1.);  // change color here
	_offset = offset;
	return (seg>0.);
}
...
void main( void ) { 
    vec2 uv = 2.*(gl_FragCoord.xy/ iResolution.y- vec2(.5*iResolution.x/iResolution.y,.5));
	...
	if (!keyToggle(68)) { // 'D' : switch on/off display mouse.xy
		...
		pos.y -= .15;	   if (display( pos, scale, 0., mouse.y*100.)) return; 
	}
...
#define FACTOR 1.5
	...
	gl_FragColor.xyz = (keyToggle(67)) ?  vec3(d) : colormap(d) ;
}
