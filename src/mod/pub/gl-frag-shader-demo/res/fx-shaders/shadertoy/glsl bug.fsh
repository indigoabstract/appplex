// https://www.shadertoy.com/view/XdsSRN
// mouse to explore the field. mod(x,y) is not always correct: sometimes it gives y !!! Here is a small test code. none-grey pixels are abnormals. tiles=32x32
// the hearth of the test is:
//
//	  vec2  uv = gl_FragCoord.xy;
//	  float v  = mod(uv.x,uv.y);
//    if (v==uv.y)    gl_FragColor = vec4(1.,0.,0.,1.) ;
//    else  		  gl_FragColor = .5*vec4(v/uv.y); 
...
//	vec2 uv = gl_FragCoord.xy/iResolution.y;
	...
	gl_FragColor += vec4(0.,0.,seg,1.);  // change color here
	_offset = offset;
	return (seg>0.);
}
...
void main(void)
{
	...
	vec2  uv   = floor(.5*gl_FragCoord.xy)-5.*mouse.xy;
	float v;
	if (isInf((uv.x-uv.x)/(uv.x-uv.x))) {gl_FragColor = vec4(0.,1.,0.,1.) ; return; }
#if 1
	v = mod(uv.x,uv.y);
#else
...
#endif
	if      (isNan(v))   gl_FragColor = vec4(0.,1.,0.,1.) ;
	else if (isInf(v))   gl_FragColor = vec4(0.,.3,1.,1.) ;
	else if (v==uv.y)    gl_FragColor = vec4(1.,0.,0.,1.) ;
	else if (abs(v)>abs(uv.y-.1))  gl_FragColor = vec4(1.,.8,0.,1.) ;
	else  		         gl_FragColor = .5*vec4(v/uv.y); 
		
    if ((mod(uv.x,32.)==0.) || (mod(uv.y,32.)==0.) )  gl_FragColor.b = 1.;   // frame
