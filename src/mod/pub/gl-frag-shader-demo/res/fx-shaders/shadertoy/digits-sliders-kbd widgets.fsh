...
bool display(vec2 pos, float scale, float offset, int number, int dot) { // dot: draw separator
	...
	gl_FragColor += vec4(seg);  // change color here
	_offset = offset;
	return (seg>0.);
}
...
{
	float R=5.;
	vec2 pix = gl_FragCoord.xy/iResolution.y;
    ...
    float k = dot(ptr-pix,ptr-pix)/(R*R*.4*.4);
		if (k<1.) 
	    { if (k>.8*.8) gl_FragColor = vec4(0.);
		     else      gl_FragColor = vec4(s,.4,0.,1.); 
		  return true;
		}
    ...
    k = dot(val-pix,val-pix)/(R*R*.4*.4);
		if (k<1.) 
	    { if (k>.8*.8) gl_FragColor = vec4(0.);
		     else      gl_FragColor = vec4(0.,.2,s,1.); 
		  return true;
		}
...
bool affSlider(vec2 p0, vec2 dp, float v)
{
	...
	y = sqrt(y);
	if (y<pt) { gl_FragColor = vec4(1.,.2,0.,1.); return true; }      // rule
	vec2 p = vec2(x-v,y);
	if (dot(p,p)<R*R) { gl_FragColor = vec4(1.,.2,0.,1.); return true; }  // button
...
void main(void)
{
	...
	if (affMouse()) return; // display mouse and state
	...
	{   // display sliders
		...
		if (affSlider(pos.xy, len.xy,(mouse.x-pos.x)/length(len))) return;
		if (affSlider(pos.yx, len.yx,(mouse.y-pos.x)/length(len))) return; 
	}
	...
	pos = vec2(.2,.8);    if (display( pos, scale, 0., mouse.x*100.)) return;
	pos.y -= STEPY*scale; if (display( pos, scale, 0., mouse.y*100.)) return; 
	pos.y -= STEPY*scale; if (display( pos, scale, 0., mouse.z*100.)) return; 
	pos.y -= STEPY*scale; if (display( pos, scale, 0., mouse.a*100.)) return; 
	pos.y -= STEPY*scale; if (display( pos, scale, 0., mod(iGlobalTime,60.))) return; 
