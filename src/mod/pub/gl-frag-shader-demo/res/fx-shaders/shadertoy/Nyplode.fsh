// https://www.shadertoy.com/view/4dfSWs
...
void main(void)
{
    ...
    // blast bar
    col.rgb = mix(col.rgb, blastBar(1.0-uv.x), step(abs(gl_FragCoord.y-blastBarPos), 3.0));
    col.rgb = mix(col.rgb, vec3(1.0), step(abs(gl_FragCoord.y-blastBarPos), 5.0)*
                  step(abs(gl_FragCoord.x - (blast-20.0)/200.0*iResolution.x), 2.0));

    // cycle time bar
    col.rgb = mix(col.rgb, vec3(0.8), step(abs(gl_FragCoord.y-timeBarPos), 12.0));
    float d = length(vec2(iResolution.x*mod(t, dur)/dur, timeBarPos)-gl_FragCoord.xy)-6.0;
