// https://www.shadertoy.com/view/ldsSWH
...
float get_distance(vec3 point) {
	...
	float water_h = water(point, iGlobalTime*speed);
...
void main(void)
{
	...
	vec3 light = vec3(-1.0, 4.0, 8.0) * rotateY(iGlobalTime);
	
	mat3 rotated = mat3(1.0); //rotateY(sin(iGlobalTime/4.0));
