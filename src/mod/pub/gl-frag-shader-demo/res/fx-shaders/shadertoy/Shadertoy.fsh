...
vec3 StyleScanline( float f )
{
	float fShade = f * 0.8 + 0.2;
	
	fShade *= mod(gl_FragCoord.y, 2.0);
...
{	
	...
	for(int iX=0; iX<AA_X; iX++)
	{
		...
		for(int y=0; y<AA_Y; y++)
		{
			...
			vec2 vUV = (gl_FragCoord.xy + vec2(fX, fY)) / iResolution.xy;
			...
			{
				vResult += vec4(StyleScanline(fShadertoy), 1.0);
			}
    ...
    gl_FragColor = vec4(vResult.xyz, 1.0);
}
