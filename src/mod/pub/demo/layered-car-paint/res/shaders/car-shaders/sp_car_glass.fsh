...
void main()
{
	...
	float intensity = 2.0 * (v4_reflected_color.r + v4_reflected_color.g + v4_reflected_color.b) / 3.0;
	v4_reflected_color.rgb *= intensity;//v4_reflected_color.a * 8.0;//u_v1_brightness_factor;
	
	vec3 v3_p_norm1_ws = normalize(m3_ns_to_ws * v3_p_norm1_ns);
	float v1_fresnel_fact1 = clamp(max(dot(v3_p_norm1_ws, -v3_cam_dir), 0.1), 0.0, 1.0);
	...
	vec3 v3_frag_color = v4_reflected_color.rgb * v3_dot_view_normal * pow(0.25, 1.0-intensity) + v4_paint_layer_color.rgb*0.05;

	gl_FragColor = vec4(v3_frag_color, 0.75);
}
