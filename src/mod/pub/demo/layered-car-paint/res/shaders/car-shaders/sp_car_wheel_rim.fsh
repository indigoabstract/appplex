...
void main()
{
	...
	vec3 v3_p_norm1_ns = v3_flake_nrm_ns;
	...
	vec3 v3_normal_ws = normalize(m3_ns_to_ws * v3_p_norm1_ns);
	v3_normal_ws = normalize(m3_ws_tf * v_v3_normal);
	...
	//v1_fresnel_fact1 = mix(v1_fresnel_fact1, v3_dot_view_normal, 0.5);
	...
	float f1 = 0.75;
	float f2 = 1.0;
	vec4 chrome1 = vec4(f1, f1, f1, 0.0);
	vec4 chrome2 = vec4(f2, f2, f2, 0.0);
	...
	//vec4 v4_paint_layer_color = v4_c2 + v4_c3;
	float v1_reflected_color_weight = (1.0 - 0.5 * v3_dot_view_normal) * 1.5;
	float v1_specular = pow(max(0.0, v3_dot_view_normal), 75.0) * 125.0;
	vec3 v3_frag_color = v4_reflected_color.rgb * v1_reflected_color_weight;
	v3_frag_color += vec3(0.1);// + v4_paint_layer_color.rgb * 0.5;
	v3_frag_color += v1_specular * vec3(1.0);
