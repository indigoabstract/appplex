...
mod_kemx::mod_kemx() : mws_mod(mws_stringify(MOD_KEMX))
{
   is_valid_expression = false;
}

mws_sp<mod_kemx> mod_kemx::nwi()
{
   return mws_sp<mod_kemx>(new mod_kemx());
}
...
void mod_kemx::unload()
{
   node_list.clear();
}

void mod_kemx::create_mws_tree_model(const vector<mws_sp<node_info> >& list, mws_sp<mws_tree_model_node> node, int& length)
{
   int size = list.size();
