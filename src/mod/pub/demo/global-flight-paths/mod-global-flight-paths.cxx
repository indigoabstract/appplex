...
namespace global_flight_paths_ns
{
   ...
   void get_hot_spot_data(float latitude, float longitude, float globe_radius, glm::vec3& position, glm::vec3& normal)
   {
      ...
      glm::vec3 mod_sphere_position = glm::vec3(sign_long * glm::sin(longitude_rad) * glm::cos(latitude_rad), glm::cos(longitude_rad) * glm::cos(latitude_rad), glm::sin(latitude_rad));
      normal = glm::normalize(mod_sphere_position);
   ...
   public:
      virtual void init()
      {
         ...
         t = 0;
         free_cam = std::make_shared<free_camera>(get_mod());
         ...
         auto scene = get_mod()->gfx_scene_inst;
...
mod_global_flight_paths::mod_global_flight_paths() : mws_mod(mws_stringify(MOD_GLOBAL_FLIGHT_PATHS)) {}

mws_sp<mod_global_flight_paths> mod_global_flight_paths::nwi()
{
   return mws_sp<mod_global_flight_paths>(new mod_global_flight_paths());
}

void mod_global_flight_paths::init_mws()
{
