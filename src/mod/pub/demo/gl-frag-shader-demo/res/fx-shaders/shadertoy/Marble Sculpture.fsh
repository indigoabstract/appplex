...
// Derived from my wax shader, a slightly more complex scene & material.
...
void main()
{
	...
	if ( Toggle(kLensFX) )
	{
		...
		col += LensFlare( ray, lightCol1, lightDir1, smoothstep(.01,.1,s1), step(t,.0) );
		col += LensFlare( ray, lightCol2*lightIntensity2, lightDir2, smoothstep(.01,.1,s2), step(t,.0) );
