...
void main(void)
{
	vec2 uv = (2.0 * gl_FragCoord.xy / iResolution.xy - vec2(1)) * vec2( 1.0, 1.0 );
	...
	//gl_FragColor = vec4( mix( vec3( 0.4, 0.35, 0.25 ), mix( 1.0, 0.4 + 0.6 * pow( clamp( edgemask, 0.0, 1.0 ), 0.4), 20.0 * softness ) * material, mask ), 1.0);
    const float gamma = 1.0 / 2.4;
    gl_FragColor = vec4( pow( hitColor.x, gamma ), pow( hitColor.y, gamma ), pow( hitColor.z, gamma ), 1.0 );
	//gl_FragColor += 1.0 * vec4( closestDist1 / softness1, closestDist2 / softness2, 0.0, 1.0);
	//gl_FragColor = vec4( 0.0 );
	//gl_FragColor += 0.5 * vec4( clamp( closestDist1 / softness1, 0.0, 1.0), clamp( closestDist2 / softness2, 0.0, 1.0 ), 0.0, 1.0);
	//gl_FragColor = vec4( vec3( 0.0 ) * ( 1.0 - binary_edge ) + binary_edge * vec3( 0.7 + 0.3 * hash(rockId + vec2(1.0)) ), 1.0);
	//gl_FragColor = vec4( vec3( 0.5 ) * min( 1.0, 1.0 - edgemask * 1000.0 ), 1.0 );
    //gl_FragColor = vec4( iMouse.z, -iMouse.z * 0.001, iMouse.z * 0.01, 1.0 );
}
