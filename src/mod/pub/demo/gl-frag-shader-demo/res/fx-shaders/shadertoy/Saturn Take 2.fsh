...
{	
	...
	float xzDist = fract(length(pos.xz)/stride);
	float angle = atan(pos.z, pos.x);
	
	//float ringdens = snoise2d(1.0, xzDist
	//float ringsSDF = -abs(xzDist-0.5) + mix(0.5, 0.0, width);
							  
	float ringsSDF = snoise2d(vec2(1.0, length(pos.xz)*1.2));
	
	// noise up the rings to make them look like rocks
	float waves = 0.05*noise(20.0*pos) + 0.01*noise(50.0*pos);
	waves = 0.;
...
float planetSDF( in vec3 p )
{
	...
	float noisePart = 0.0 * noise(5.0*p) * sin(3.0*bandFreq*dot(normalize(p),spotDir));
...
void main( void )
{
    ...
    // sky	 
	vec3 col = vec3(0.05,0.01,0.01) - rd.y*0.4;
    ...
    if( t>0.0 )
	{
		...
		if( cd < satradius*1.1 )
		{
			...
			// add in some blue as we go lower
			albedo += vec3(0,0.5,0.8) * smoothstep(0., -2., pos.y);
			// make the center glow a bit
			albedo += vec3(1,0.2,0.0) * smoothstep(1.5, 0.0, abs(pos.y)) * pow(dot(nor,-rd),2.0);
		...
		{
			// shade rings
			vec3 albedo = mix( vec3(0.8,0.6,0.0), vec3(0.7,0.7,0.0), snoise2d( vec2(0.0, cd*1.2) ) );
			// add in some blue as we go lower
			albedo += vec3(0,0.5,0.8) * smoothstep(0., -2., pos.y);
			// make the center glow a bit
			albedo += vec3(1,0.2,0.0) * smoothstep(1.5, 0.0, abs(pos.y)) * pow(dot(nor,-rd),2.0);
