...
// C: toggles greyscale vs colors G: toggles grid vs dots D: toggles deformations vs displacement S: toggles stereo (horiz displ only) vs radial disp M: mark central dots SPACE: toggles mouse tune center or color #define NB : number of dots vertically
float t = iGlobalTime;
...
void main(void) {
	...
	if (keyToggle(77) && floor(.25*NB*disp(uv,center) +.25)/NB==0.)
		grey = !grey;
