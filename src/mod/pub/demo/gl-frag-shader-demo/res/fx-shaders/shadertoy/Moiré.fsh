// https://www.shadertoy.com/view/MdsSRM

// iChannel2: kb

// A: anti aliasing C: toggles colors M: draw mode = max vs sum mouse: translate. Z: mouse.y zoom instead. S: mouse.x tunes wires step instead.
...
void main(void)
{
	...
	vec2 uv = gl_FragCoord.xy-.5-iResolution.xy/2.;
	vec2 trans = vec2(0.);
	...
	gl_FragColor = (!keyToggle(67)) ? vec4(col.r+col.g) : vec4(col.r,col.g,0.,1.);
}
