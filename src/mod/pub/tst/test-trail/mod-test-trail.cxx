...
public:
   ...
   void load_trail_data()
   {
      ...
      if (file->exists())
      {
         ...
         {
            float x = dr.r.read_f32();
            float y = dr.r.read_f32();
            float z = dr.r.read_f32();
            tube_pos[i] = glm::vec3(x, y, z);
         }

         for (int i = 0; i < tube_ts.size(); i++)
         {
            int ts = dr.r.read_i32();
            tube_ts[i] = ts;
         }
      }
      else
...
mod_test_trail::mod_test_trail() : mws_mod(mws_stringify(MOD_TEST_TRAIL)) {}

mws_sp<mod_test_trail> mod_test_trail::nwi()
{
   return mws_sp<mod_test_trail>(new mod_test_trail());
}
...
void mod_test_trail::load()
{
   p = mws_sp<mod_test_trail_impl>(new mod_test_trail_impl());
...
void mod_test_trail::receive(mws_sp<mws_dp> idp)
{
   if (!idp->is_processed())
   {
