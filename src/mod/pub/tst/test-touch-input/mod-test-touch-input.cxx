...
{
   return mws_sp<mod_test_touch_input>(new mod_test_touch_input());
}
   ...
   public:
      ...
      void add_objects()
      {
         ...
         for (auto m : obj_vect)
         {
            ...
            m->camera_id_list.push_back(get_mod()->mws_cam->camera_id());
            scene->attach(m);
         }

         set_obj_scaling(1.5f);
         scene->attach(picking_dgb_q2d);
         get_mod()->mws_cam->sort_function = get_mod()->mws_cam->z_order_sort_function;
      }
...
void mod_test_touch_input::init_mws()
{
   mws_root->new_page<mod_test_touch_input_ns::main_page>();
