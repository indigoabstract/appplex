...
public: 
     ...
     */ 
    std::shared_ptr<postream> get_stdin() const 
    { 
        BOOST_ASSERT(stdin_); 
     ...
     */ 
    std::shared_ptr<pistream> get_stdout() const 
    { 
     ...
     */ 
    std::shared_ptr<pistream> get_stderr() const 
    { 
     ...
     *         it terminates. 
     */ 
    status wait(unsigned long millisecondsToWait = 0xFFFFFFFF) 
    { 
#if defined(BOOST_POSIX_API) 
        int s; 
        ...
        return status(s); 
#elif defined(BOOST_WINDOWS_API) 
        ::WaitForSingleObject(process_handle_.get(), millisecondsToWait); 
        DWORD code; 
...
private: 
     ...
     * data stream. 
     */ 
    std::shared_ptr<postream> stdin_; 
     ...
     * data stream. 
     */ 
    std::shared_ptr<pistream> stdout_; 
     ...
     * data stream. 
     */ 
    std::shared_ptr<pistream> stderr_; 

#if defined(BOOST_WINDOWS_API) 
    /** 
     * Process handle owned by RAII object. 
     */ 
    std::shared_ptr<void> process_handle_; 
#endif 
}; 
